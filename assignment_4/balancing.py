...
class Decentralised_bot:
    def __init__(self):
        ...
        self.rbot = None
        self.bot = None
        self.bot_sub = rospy.Subscriber('/odom', Odometry, callback_odom) #topic name fixed
        self.lbot_sub = rospy.Subscriber('/left_odom', Odometry, callback_left_odom) #topic name fixed
        self.rbot_sub = rospy.Subscriber('/right_odom', Odometry, callback_left_odom) #topic name fixed
...
while (~rospy.is_shutdown()) and (bot_i.num < 10): #replace with balancing reached?
