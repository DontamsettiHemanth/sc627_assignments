...
def computeDistancePointToPolygon(P, q):
    ...
    for i in range(len(P)):
        ...
        dists.append(d)
    return min(dists)
def computeDistance_P_PointToPolygon(P, q):
    # check inside polygon
    if insidePoly(P, q):
        return 0 #,TODO
    # outside polygon
    num_P = len(P)
    min_dist, min_w = computeDistancePointToSegment(q, P[num_P-1], P[0])
    min_i = num_P-1
    for i in range(num_P):
        d, w = computeDistancePointToSegment(q, P[i], P[(i+1) % num_P])
        if min_dist > d:
            min_dist = d; min_i = i; min_w = w

    d0 = (P[(min_i+1) % num_P]-P[min_i])
    d0 = d0.multi(1/d0.norm())
    n0 = point(-d0.y, d0.x)
    if min_w == 0:
        return min_dist, q + n0.multi(min_dist/n0.norm())
    elif min_w == 1:
        return min_dist, P[min_i]
    elif min_w == 2:
        return min_dist, P[(min_i+1) % num_P]
